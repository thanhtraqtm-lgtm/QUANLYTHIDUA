...
export default function Header({
  ...
  return (
    <header className="bg-white border-b border-indigo-100/80 sticky top-0 z-40 shadow-sm relative overflow-hidden transition-all duration-300">
      {/* Nền đồ họa (Khôi phục nguyên bản) */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 relative z-10">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between items-start gap-4 pb-4">
          <div className="flex items-center space-x-3.5">
            ...
            <div>
              <h1 className="text-md sm:text-2xl font-black tracking-tight text-slate-900">HỆ THỐNG QUẢN LÝ THI ĐUA</h1>
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Thống kê Hưng Yên 2026</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={onExportExcel} className="px-3 py-2 text-xs bg-emerald-600 text-white font-black rounded-xl hover:bg-emerald-700">Xuất Excel</button>
            <button onClick={onLogout} className="p-2 text-slate-400 hover:text-rose-600"><LogOut className="w-4 h-4" /></button>
          </div>
        </div>

        {/* MENU ĐÃ CHỈNH SỬA: Border dưới, text đen đậm, gọn gàng */}
        <div className="flex overflow-x-auto gap-4 border-t border-slate-200 pt-3">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            const isActive = activeTab === tab.id;
            return (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`flex items-center gap-2 pb-2 px-1 text-[11px] font-black transition-all border-b-2 whitespace-nowrap ${
