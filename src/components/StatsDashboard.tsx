...
interface StatsDashboardProps {
  ...
  onClickOverdue?: () => void;
  onClickCompletionRate?: () => void;
  onClickEmulationScore?: () => void;
  onClickQualityScore?: () => void;
  onClickTimeScore?: () => void;
  onResetData?: () => void;
}
...
export default function StatsDashboard({ 
  ...
  return (
    <div className="space-y-6" id="stats-dashboard-container">
      ...
      {/* Alert banner when there are 0 submissions to offer mock data generation */}
      {totalSubmittedCount === 0 && onResetData && (
        <div className="bg-amber-50 border-2 border-amber-200 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm animate-pulse">
          ...
          <button
            ...
            className="bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow-sm transition-all shrink-0 cursor-pointer hover:shadow-md"
          >
            Nạp số liệu mô phỏng mẫu
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div 
          onClick={onClickCompletionRate}
          className={`bg-white p-5 rounded-2xl border-2 border-sky-100 shadow-md flex items-center space-x-4 hover:translate-y-[-2px] ${onClickCompletionRate ? 'cursor-pointer hover:border-sky-300 group' : ''} transition-all duration-200 relative overflow-hidden`}
        >
          <div className="p-3 bg-sky-50 rounded-xl text-sky-600 group-hover:bg-sky-105 transition-colors">
          ...
          <div className="absolute bottom-0 right-0 w-12 h-1 bg-sky-500 rounded-bl-full group-hover:h-2 transition-all" />
        </div>

        {/* Metric 2 - Điểm Thi Đua TB */}
        <div 
          onClick={onClickEmulationScore}
          className={`bg-white p-5 rounded-2xl border-2 border-slate-100 shadow-md flex items-center space-x-4 hover:translate-y-[-2px] ${onClickEmulationScore ? 'cursor-pointer hover:border-indigo-300 group' : ''} transition-all duration-200 relative overflow-hidden`}
        >
          <div className="p-3 bg-indigo-50 rounded-xl text-indigo-600 group-hover:bg-indigo-105 transition-colors">
            <Award className="w-5 h-5" />
          </div>
          <div>
            <p className="text-[11px] text-slate-800 font-black font-sans tracking-wide uppercase">Điểm Thi đua TB</p>
            <h3 className="text-xl font-black font-sans text-indigo-700 leading-none mt-1.5">{avgScore.toFixed(1)}</h3>
            <p className="text-[10px] text-slate-600 mt-1 font-sans font-bold">
              Chỉ số tổng hợp thi đua
            </p>
          </div>
          <div className="absolute bottom-0 right-0 w-12 h-1 bg-indigo-500 rounded-bl-full group-hover:h-2 transition-all" />
        </div>

        {/* Metric 3 - Điểm Chất lượng TB */}
        <div 
          onClick={onClickQualityScore}
          className={`bg-white p-5 rounded-2xl border-2 border-amber-100 shadow-md flex items-center space-x-4 hover:translate-y-[-2px] ${onClickQualityScore ? 'cursor-pointer hover:border-amber-300 group' : ''} transition-all duration-200 relative overflow-hidden`}
        >
          <div className="p-3 bg-amber-50 rounded-xl text-amber-600 group-hover:bg-amber-105 transition-colors">
            <TrendingUp className="w-5 h-5" />
          </div>
          <div>
            <p className="text-[11px] text-slate-800 font-black font-sans tracking-wide uppercase">Điểm Chất lượng TB</p>
            <h3 className="text-xl font-black font-sans text-amber-700 leading-none mt-1.5">{avgQualityScore.toFixed(1)}</h3>
            <p className="text-[10px] text-slate-600 mt-1 font-sans font-bold">
              Đánh giá từ Phòng nghiệp vụ
            </p>
          </div>
          <div className="absolute bottom-0 right-0 w-12 h-1 bg-amber-500 rounded-bl-full group-hover:h-2 transition-all" />
        </div>

        {/* Metric 4 - Điểm Thời gian TB */}
        <div 
          onClick={onClickTimeScore}
          className={`bg-white p-5 rounded-2xl border-2 border-emerald-100 shadow-md flex items-center space-x-4 hover:translate-y-[-2px] ${onClickTimeScore ? 'cursor-pointer hover:border-emerald-300 group' : ''} transition-all duration-200 relative overflow-hidden`}
        >
          <div className="p-3 bg-emerald-50 rounded-xl text-emerald-600 group-hover:bg-emerald-105 transition-colors">
            <Clock className="w-5 h-5" />
          </div>
          <div>
            <p className="text-[11px] text-slate-800 font-black font-sans tracking-wide uppercase">Điểm Thời gian TB</p>
            <h3 className="text-xl font-black font-sans text-emerald-700 leading-none mt-1.5">{avgTimeScore.toFixed(1)}</h3>
            <p className="text-[10px] text-slate-600 mt-1 font-sans font-bold">
              Đạt tỉ lệ đúng hạn: <strong className="text-emerald-800 font-extrabold">{onTimeRate.toFixed(0)}%</strong>
            </p>
          </div>
          <div className="absolute bottom-0 right-0 w-12 h-1 bg-emerald-500 rounded-bl-full group-hover:h-2 transition-all" />
        </div>

        {/* Metric 5 - Cảnh báo Chậm trễ */}
        <div 
          onClick={onClickOverdue}
          className={`bg-white p-5 rounded-2xl border-2 border-rose-100 shadow-md flex items-center space-x-4 hover:translate-y-[-2px] ${onClickOverdue ? 'cursor-pointer hover:border-rose-200 group' : ''} transition-all duration-200 relative overflow-hidden`}
        >
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-600 mt-1 font-sans font-bold">
              Nhấp xem chi tiết danh sách quá hạn
            </p>
          </div>
          <div className="absolute bottom-0 right-0 w-12 h-1 bg-rose-500 rounded-bl-full group-hover:h-2 transition-all" />
        </div>
            ...
            {regionStats.map((reg, index) => (
              <div key={reg.name} className="space-y-2">
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <span className="font-mono text-xs font-extrabold text-slate-800">{reg.avgScore.toFixed(1)}</span>
                </div>
              </div>
            ))}
          </div>
              ...
              {topUnits.map((item, idx) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <span className="font-mono text-xs font-extrabold text-slate-800">{item.Diem_Thi_Dua.toFixed(1)}</span>
