...
export default function ReportBrowser({ 
  ...
  const [selectedGradeStatus, setSelectedGradeStatus] = useState<string>('ALL');
  ...
  const handleOpenChaoModal = (sub: ReportSubmission) => {
    ...
    setEditDiemChatLuong(sub.Diem_Chat_Luong !== null ? sub.Diem_Chat_Luong : sub.Diem_Dinh_Muc);
    setEditNhanXet(sub.Nhan_Xet || '');
  };

  // Direct calculation of points
  const handleApplyPreset = (percent: number, maxScore: number) => {
    const scoreVal = Math.round((percent / 100) * maxScore * 10) / 10;
  ...
  const handleSaveAssessment = () => {
    ...
    if (editNgayNop) {
      ...
      // Calculate Diem_Thoi_Gian (-2 pts delay per day, capped to min 0)
      if (actualGaps > 0) {
        data.Diem_Thoi_Gian = Math.max(0, editingReport.Diem_Dinh_Muc - (actualGaps * 2));
      } else {
        data.Diem_Thoi_Gian = editingReport.Diem_Dinh_Muc;
      }
    } else {
      data.Ngay_Nop = null;
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-xs space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            >
              ...
              <option value="LATE">Chậm tiến độ / Quá hạn</option>
            </select>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between text-[11px] font-mono text-slate-400 pt-1">
          <div className="flex items-center space-x-2">
            ...
            <span>•</span>
            <span>Bình quân: <strong className="text-sky-600">{avgPoints} đ</strong></span>
          </div>
                    ...
                    <tr key={row.ID} className="hover:bg-slate-50/40 transition-colors duration-100">
                      ...
                      <td className="py-3.5 px-3 text-center">
                        <span className="font-mono font-bold text-xs text-sky-600 bg-sky-50 px-2 py-1 rounded">
                          {row.Diem_Thoi_Gian !== null ? `${row.Diem_Thoi_Gian}đ` : '0đ'}
                        </span>
                      </td>
                      ...
                      <td className="py-3.5 px-3 text-center">
                        <span className="font-mono font-black text-xs text-indigo-700 bg-indigo-50 px-2 py-1.5 rounded-lg border border-indigo-100">
                          {row.Tong_Diem !== null ? `${row.Tong_Diem}đ` : '0đ'}
                        </span>
                      </td>
                        ...
                        <div className="flex flex-col items-center space-y-1">
                          ...
                          {row.Diem_Chat_Luong !== null ? (
                            <span className="font-mono font-bold text-[11px] text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded leading-none border border-emerald-150">
                              Chuyên môn đạt: {row.Diem_Chat_Luong}đ
                            </span>
                          ) : (
              ...
              <div className="p-5 space-y-4 text-xs font-sans">
                ...
                <div className="grid grid-cols-3 gap-2.5 text-center">
                  <div className="p-2.5 bg-slate-50 border border-slate-100 rounded-xl">
                    ...
                    <span className="text-md font-black text-slate-800 block mt-0.5">{editingReport.Diem_Dinh_Muc}đ</span>
                  </div>
                  <div className="p-2.5 bg-sky-50 rounded-xl border border-sky-100">
                    ...
                    <span className="text-md font-black text-sky-800 block mt-0.5">
                      {editingReport.Diem_Thoi_Gian !== null ? `${editingReport.Diem_Thoi_Gian}đ` : '0đ'}
                    </span>
                  </div>
                  <div className="p-2.5 bg-indigo-50 rounded-xl border border-indigo-150">
                    ...
                    <span className="text-md font-black text-indigo-800 block mt-0.5">
                      {editingReport.Diem_Chat_Luong !== null ? `${editingReport.Diem_Chat_Luong}đ` : 'Chưa chấm'}
                    </span>
                  </div>
                </div>
                  ...
                  <div className="space-y-4 border-t border-slate-100 pt-3">
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between items-center text-[10px] text-slate-400 font-bold uppercase font-mono">
                        <span>* Điểm chất lượng chuyên môn (Tối đa {editingReport.Diem_Dinh_Muc}đ):</span>
                        <span className="text-indigo-600">Đạt {editDiemChatLuong}đ</span>
                      </div>
                      
                      <input
                        type="number"
                        min="0"
                        max={editingReport.Diem_Dinh_Muc}
                        step="0.5"
                        value={editDiemChatLuong}
                        onChange={(e) => setEditDiemChatLuong(Math.min(editingReport.Diem_Dinh_Muc, Math.max(0, parseFloat(e.target.value) || 0)))}
